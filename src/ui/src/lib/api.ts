...
function runtimeToolCatalogPrompt(runtimes: AgentRuntime[]): string {
  ...
  return [
    ...
    ...runtimes.map((runtime) => {
      ...
      return `- ${runtime.id}: ${tools || "no explicit OAP-managed tools"}`;
    }),
  ].join("\n");
}
