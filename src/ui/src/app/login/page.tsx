...
function LoginForm() {
  ...
  return (
    <div className="min-h-dvh flex w-full bg-background text-foreground overflow-hidden selection:bg-emerald-500/20">
      {/* Left Column: Brand & Security Architecture Showcase (Hidden on Mobile) */}
      <div className="hidden lg:flex lg:w-1/2 relative flex-col justify-between p-12 border-r border-border/70 bg-card overflow-hidden">
        ...
        <div className="relative z-10">
          <OapLogo size={32} showText subtitle="Control Plane v2.0" />
        </div>

        <div className="relative z-10 space-y-8 my-auto max-w-lg">
          <div className="space-y-3">
            <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3 py-1 text-xs font-mono text-emerald-600 dark:text-emerald-400">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              统一集成 LLM 网关、MCP 扩展生态、系统法则约束与凭证保险库。在零信任沙箱中实时调度智能体任务。
            </p>
          </div>
        ...
        <div className="relative z-10 flex items-center justify-between text-xs text-muted-foreground font-mono">
          <span>OAP SECURITY ENCLAVE</span>
          <span>SYSTEM READY · ACTIVE</span>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-between p-6 sm:p-12 bg-background">
        ...
        <div className="w-full max-w-sm mx-auto my-auto space-y-6">
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              输入你的 Master Access Key 以验证身份并进入智能体控制中心。
            </p>
          </div>
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <Label htmlFor="key" className="text-xs font-mono font-medium text-muted-foreground uppercase tracking-wider">
                  Access Key (访问密钥)
                </Label>
              </div>
            ...
            {error && (
              <div className="rounded-xl border border-destructive/40 bg-destructive/10 p-3 text-xs text-destructive flex items-center gap-2 font-mono">
          ...
          <div className="rounded-xl border border-border/60 bg-muted/30 p-3.5 text-xs text-muted-foreground space-y-1">
            ...
            <p className="text-[11px] leading-relaxed">
              可以使用全局管理员主密钥，或在系统后台分发的个人 Key 进行认证登录。
            </p>
          </div>
        </div>

        <div className="text-center text-[11px] font-mono text-muted-foreground py-2">
          Open Agent Platform &copy; 2026. All rights reserved.
        </div>
      </div>
    </div>
  );
