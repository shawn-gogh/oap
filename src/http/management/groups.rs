...
use crate::{
    db::managed_agents::{groups, users},
    errors::GatewayError,
...
pub async fn list(
    ...
    let groups = groups::repository::list(&pool, query.query.as_deref()).await?;
...
pub async fn list_members(
    ...
    let (pool, _) = admin_pool(&state, &headers).await?;
    if groups::repository::find(&pool, &group_id).await?.is_none() {
...
pub async fn add_member(
    ...
    let (pool, added_by) = admin_pool(&state, &headers).await?;
    if !groups::repository::find(&pool, &group_id)
        .await?
...
pub async fn delete_member(
    ...
    let (pool, _) = admin_pool(&state, &headers).await?;
    Ok(Json(
        groups::members::delete(&pool, &group_id, &user_id).await?,
    ))
}
