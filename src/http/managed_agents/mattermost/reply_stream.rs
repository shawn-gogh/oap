...
use super::{
    ...
    reply_format::{runtime_status, runtime_text},
    reply_storage::{closed_text, final_text, persisted_assistant_text_after},
    web_api,
};

pub(super) struct MattermostReply<'a> {
    state: &'a AppState,
    pool: &'a PgPool,
    bot_token: &'a str,
    server_url: &'a str,
...
pub(super) struct MattermostReplyParams<'a> {
...
impl<'a> MattermostReply<'a> {
    pub(super) fn new(params: MattermostReplyParams<'a>) -> Self {
        Self {
            state: params.state,
            pool: params.pool,
    ...
    async fn update_active(&mut self, text: &str) -> Result<(), GatewayError> {
        self.post_id = Some(
            web_api::upsert_post(web_api::UpsertPostParams {
                client: &self.state.http,
                server_url: self.server_url,
                bot_token: self.bot_token,
                channel_id: self.channel_id,
                root_id: self.root_id,
                post_id: self.post_id.as_deref(),
                text,
            })
            .await?,
        );
